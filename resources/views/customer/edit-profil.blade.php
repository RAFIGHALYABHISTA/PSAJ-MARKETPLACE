  ...
  <div class="w-full max-w-4xl bg-white rounded-[40px] shadow-[0_20px_60px_rgba(91,44,4,0.1)] overflow-hidden border border-stone-200">
    ...
    <div class="relative -mt-12 text-center mb-12">
      ...
      <div class="space-y-4">
        @if(auth()->check())
          <!-- PROFIL -->
          <a href="{{ url('/edit-profil1') }}" class="group flex items-center justify-between p-4 rounded-[25px] bg-stone-50 hover:bg-[#5B2C04] transition-all duration-300 border border-stone-100">
          ...
          <a href="javascript:void(0)" onclick="openSettings()" class="group flex items-center justify-between p-4 rounded-[25px] bg-stone-50 hover:bg-[#5B2C04] transition-all duration-300 border border-stone-100">
            <div class="flex items-center gap-4">
              <div class="w-10 h-10 rounded-2xl bg-white flex items-center justify-center text-[#5B2C04] group-hover:bg-white/20 group-hover:text-white transition-colors">
          ...
          @if(!in_array(auth()->user()->role, ['admin', 'superadmin', 'afiliator']))
            ...
            <div class="mt-8 p-6 rounded-[30px] bg-[#FFEADB] border relative overflow-hidden">
              ...
              <a href="{{route('afiliator.register')}}" class="text-sm font-bold text-green-800 hover:underline">
                Mulai Sekarang →
              </a>
            </div>
